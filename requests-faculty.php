...
<li class="nav-item dropdown" id="requests-student">
  ...
  <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
    ...
    if (isset($numOfRequests)){
        ...
        $supervisorId = $_SESSION['usrId'];
        $sql = "SELECT * , studentImage from faculty_student_request JOIN student_group JOIN student ON student.studentId = student_group.leaderId WHERE faculty_student_request.facultyId = '$supervisorId' AND student_group.groupId = faculty_student_request.groupId";
        $result = $link->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                <div class="media dropdown-item">
                    <img src="<?php if (isset($row['studentImage'])){
                             echo 'public/profile_images/'.$row['studentImage'];
                                 }else {echo 'public/profile_images/dummy.png';}?>" alt="User Avatar" class="img-size-50 mr-3 img-circle">
