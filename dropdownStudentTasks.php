...
$sql = "SELECT * FROM notification WHERE batchId ='$batchId' AND studentId = '$studentId'";
$result = $link->query($sql);


if ($result->num_rows > 0) {
  $read_tasks = $result->num_rows;
}else{
  $read_tasks = 0;
}
...
<li class="nav-item dropdown">
  <a class="nav-link" data-toggle="dropdown" href="#">
    ...
    <span class="badge badge-warning navbar-badge"><?php echo $num_of_tasks-$read_tasks ;?></span>
  </a>
    ...
    if ($result->num_rows > 0) {
     ...
     while ($row = $result->fetch_assoc()) { ?>
      <a href="<?php echo "studentTask.php?details=".$row['taskId'];?>" target="_blank" class="dropdown-item">
        <i class="fas fa-envelope mr-2"></i> <?php echo $row['taskName'];?>
