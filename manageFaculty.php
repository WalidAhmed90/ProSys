...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['btnEditFaculty'])){
        ...
        if ($_POST['name']!="" && $_POST['designation']!="" && $_POST['email']!=""  && $_POST['quota'] !="" ){
            ...
            // prepare and bind
            $stmt = $link->prepare("UPDATE  faculty JOIN work_load ON faculty.facultyId= work_load.facultyId SET facultyName = ?, designation = ?,facultyEmail = ?, facultyPhoneNo = ?, facultyPassword = ? ,  totalLoad =?, isActive=? WHERE faculty.facultyId = ?");
            $stmt->bind_param("sssssiii", $name, $designation,$email,$contact,$secure_pass, $quota,$isActive,$editId);
