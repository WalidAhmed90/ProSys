...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
     ...
     */
    if (isset($_POST['btnEditRequest'])){
        //EDIT Request
        $RequestId = filter_input(INPUT_POST,'editId',FILTER_SANITIZE_NUMBER_INT);
        ...
        if ($_POST['status'] != "" && $_POST['meetingTitle'] != ""){
            ...
            $sql = "UPDATE meeting_Requests SET meeting_title = '$title' , meeting_status='$status' , comments='$comments' WHERE id='$RequestId' ";
    ...
    //ADD NEW Request
    if (isset($_POST['addNewRequestBtn'])){
        //Validations
        if (isset($_POST['meetingTitle']) && isset($_POST['groupId']) ){
            ...
            $sql = "INSERT INTO meeting_Requests (supervisor_id, group_id, meeting_title, meeting_dtm, meeting_status)VALUES ('$supervisorId', '$groupId', '$meetingTitle', '$dateTime','1')";
            if ($link->query($sql) === TRUE) {
                header('Location:' . $_SERVER['PHP_SELF'] . '?status=t&id='.$groupId);
            } else {
                header('Location:' . $_SERVER['PHP_SELF'] . '?status=f&id='.$groupId);
    ...
    if(isset($_POST['btnAddComments'])){
        ...
        if (isset($_POST['comments']) && $_POST['comments'] != ""){
            ...
            $sql = "UPDATE meeting_Requests SET comments = '$comments' WHERE id='$RequestId' ";
            if ($link->query($sql) === TRUE) {
                header('Location:' . $_SERVER['PHP_SELF'] . '?status=t&id='.$groupId);
            } else {
                header('Location:' . $_SERVER['PHP_SELF'] . '?status=f&id='.$groupId);
    ...
    if (isset($_POST['btnDelete'])){
        ...
        $sql = "DELETE FROM meeting_Requests WHERE id= '$RequestId' LIMIT 1";
        if ($link->query($sql) === TRUE) {
            header('Location:' . $_SERVER['PHP_SELF'] . '?status=t&id='.$groupId);
        } else {
            header('Location:' . $_SERVER['PHP_SELF'] . '?status=f&id='.$groupId);
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="float-right">
                            <a href="<?php echo $_SERVER['PHP_SELF'] . '?add'; ?>" class="btn  btn-primary  "><i class="fa fa-plus"></i> New Request</a>
                            <br>
                            <br>
                        </div>
                    </div>
                        ...
                        <?php if ($groupCheck == true){ ?>
                                ...
                                * ******************/?>
                                <div class="card no-border">
                                    <div class="card-header">
                                        <h3 class="card-title">Add New Meeting Request</h3>
                                    </div>
                                    <!-- /.card-header -->
                                    <!-- form start -->
                                    <form role="form" id="addNewRequest" name="addNewRequest" method="post" data-toggle="validator">
                                        <div class="card-body">
                                        ...
                                        <div class="card-footer">
                                            ...
                                            <?php //echo $_SERVER['PHP_SELF'];?><!--" class="btn btn-default ">Cancel</a>-->
                                            <button type="submit" name="addNewRequestBtn" class="btn btn-primary float-right" onclick="return confirm('Are you sure?')">Submit</button>
                                        </div>
                                    </form>
                                    <!-- /.card-body -->
                                </div>
                            ...
                            <?php if (isset($_GET['edit']) && is_numeric($_GET['edit']) && strlen($_GET['edit']) > 0 ){
                                ...
                                * ******************/
                                ...
                                $sql = "SELECT * from meeting_Requests WHERE supervisor_id='$supervisorId' AND id='$id' LIMIT 1";
                                $result = $link->query($sql);
                                if ($result->num_rows > 0) {
                                    while($row = $result->fetch_assoc()) {
                                    ...
                                    <div class="card no-border">
                                        <div class="card-header">
                                            <h3 class="card-title">Edit Meeting Request: <?php echo $meetingTitle?></h3>
                                        </div>
                                        <div class="card-body">
                                            <!-- form start -->
                                            <form id="editRequests" name="editRequests" action="" method="post" data-toggle="validator">
                                            ...
                                            <div class="card-footer">
                                                ...
                                                <button type="submit" form="editRequests" name="btnEditRequest"  class="btn btn-primary float-right">Submit</button>
                                            </div>
                                        </div>
                                    </div>
                                    <?php
                                    ...
                                    <div class="card no-border">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="card-footer">
                                                <button onclick="goBack()" class="btn btn-default">Back</button>
                                                <?php //echo $_SERVER['PHP_SELF'];?><!--" class="btn btn-default">Back</a>-->
                                                <button type="submit" form="addComments" name="btnAddComments"  class="btn btn-primary float-right">Submit</button>
                                            </div>
                                        </div>
                                    </div>
                                    <?php
                                ...
                                <div class="card no-border">
                                    ...
                                    if (isset($_GET['id']) && is_numeric($_GET['id']) && strlen($_GET['id'])>0){ ?>
                                        ...
                                        <div class="card-body  table-responsive">
                                            <table id="meetingRequests" class="table table-head-fixed text-nowrap table-striped">
                                                <thead>
                                                    <tr>
                                                        <th>Group</th>
                                                        <th>Title</th>
                                                ...
                                                $sql = "SELECT id from meeting_Requests WHERE supervisor_id='$supervisorId' AND group_id='$groupId' LIMIT 1";
                                                $result = $link->query($sql);
                                                if ($result->num_rows > 0) {
                                                    $sql = "SELECT * from meeting_Requests WHERE supervisor_id = '$supervisorId' AND group_id = '$groupId' ORDER BY created_dtm DESC";
                                                    $result = $link->query($sql);
                                                    while($row = $result->fetch_assoc()) { ?>
                                                        <tr>
                                                            ...
                                                            ?></td>
                                                            <form id="RequestActions" name="RequestActions" method="post" data-toggle="validator">
                                                                <th><?php
                                                                $status =$row['meeting_status'];
                                                                if ($status == 'Pending'){ ?>
