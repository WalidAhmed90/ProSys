	...
	**********************************/
	...
	if (isset($_POST['btnUpgradefyp'])){
		...
		if ($link->query($sql) === TRUE) {
			...
			if ($link->query($sql) === TRUE) {
				$sql = "UPDATE batch_settings SET fyp1_grading = 0 WHERE batchId='$batchId' ";
				if ($link->query($sql) === TRUE) {
    ...
    *****************************/
    ...
    if (isset($_POST['btnDeactivate'])){
    	...
    	if ($link->query($sql) === TRUE) {
    		...
    		if ($link->query($sql) === TRUE) {
    			$sql = "DELETE FROM `notification` WHERE batchId= '$batchId' ";
						...
						<?php if (isset($_GET['settings']) && is_numeric($_GET['settings']) && strlen($_GET['settings']) > 0 ) {
							...
							<div class="card card-primary card-outline">
								<div class="card-header with-border">
									<h3 class="card-title"><i class="fa fa-cog" aria-hidden="true"></i> Setting : <?php echo $batchName; ?></h3>
								</div>
									...
									<div class="card-body table-responsive">
										<form action="" method="post" onsubmit="return confirm('Are you sure  ?');" data-toggle="validator">
											...
											<ul class="todo-list ui-sortable">
												<li class="">
													...
													<span class="text">Add this batch to Project Repository
													</span>
										...
										<?php if ($isActive == 1) {
											...
											<table class="table table-striped">
												...
												* Show batch settings
												*/
												$sql = "SELECT * FROM batch JOIN batch_settings ON batch.batchId = batch_settings.batchId WHERE batch_settings.batchId = '$batchId' ";
												$result = $link->query($sql);
												if ($result->num_rows > 0) {

													// output data of each row
													while($row = $result->fetch_assoc()) { ?>
														<?php if ($row['fypPart'] == 1) { ?>
															<tr>
																<form action="" method="post">
																	...
																	<td><button type="submit" class="btn btn-info btn-sm" name="btnfyp1grading">Submit</button></td>
																</form>
															</tr>
															<?php
														} ?>
														<?php if ($row['fypPart'] == 2) { ?>
															<tr>
																<form action="" method="post">
																	<input type="hidden" name="batchId" value="<?php echo $batchId;?>">
																	...
																	<td>
																		<select name="fyp2grading" id="fyp2grading">
																			...
																			<option value="1"  <?php if ($row['fyp2_grading'] == 1){echo "selected";}?>>Allowed</option>
																		</select>
																	</td>
																	<td><button type="submit" class="btn btn-info btn-sm" name="btnfyp2grading">Submit</button></td>
																</form>
															</tr>
															<?php
														} ?>
										...
										if ($result->num_rows > 0) {
											...
											while($row = $result->fetch_assoc()) { ?>
												<tr>
													...
													?></td>
													<td>
														<a href="<?php echo $_SERVER['PHP_SELF'].'?settings='.$row['batchId']?>" class="btn btn-primary btn-sm" ><i class="fa fa-cog" aria-hidden="true"></i> Settings</a>
													</td>
												</tr>
												<?php
											}
